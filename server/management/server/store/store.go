...
type Store interface {
	...
	// Cross-network expose methods
	GetAccountCrossNetworkExposes(ctx context.Context, accountID string) ([]*proto.CrossNetworkExpose, error)
	GetCrossNetworkExposeByID(ctx context.Context, exposeID string) (*proto.CrossNetworkExpose, error)
	CreateCrossNetworkExpose(ctx context.Context, expose *proto.CrossNetworkExpose) error
	UpdateCrossNetworkExpose(ctx context.Context, expose *proto.CrossNetworkExpose) error
	DeleteCrossNetworkExpose(ctx context.Context, exposeID string) error
	UpdateCrossNetworkExposePorts(ctx context.Context, exposeID string, ports []uint32) error
	GetExpiredCrossNetworkExposes(ctx context.Context, currentTime uint64) ([]*proto.CrossNetworkExpose, error)
	RenewCrossNetworkExposeTTL(ctx context.Context, exposeID string, ttlHours uint32) error
	EnableCrossNetworkExpose(ctx context.Context, exposeID string) error
	DisableCrossNetworkExpose(ctx context.Context, exposeID string) error
	CleanExpiredCrossNetworkExposes(ctx context.Context, maxBatch int) (int64, error)
	AddCrossNetworkExposeToNetworkMap(ctx context.Context, accountID string, networkMap *proto.NetworkMap) error
	GetCrossNetworkPeersConfig(ctx context.Context, peerID string) ([]*proto.CrossNetworkPeerConfig, error)
}
