...
import (
	...
	"github.com/netbirdio/netbird/management/server/cert"
)
...
// createCertificateManager creates and configures the certificate manager
func createCertificateManager(cfg *nbconfig.Config, certDir string) *cert.CertificateManager {
	if cfg.CloudflareConfig == nil {
		log.Info("Cloudflare configuration not found, certificate manager will not be available")
		return nil
	}

	log.Infof("Initializing Cloudflare certificate manager for zone %s", cfg.CloudflareConfig.ZoneID)

	cloudflareClient := cloudflare.NewCloudflareClient(
		cfg.CloudflareConfig.APIKey,
		cfg.CloudflareConfig.Email,
		cfg.CloudflareConfig.ZoneID,
	)

	// Set certificate directory
	if certDir == "" {
		certDir = "/var/lib/netbird/certificates"
	}
...
var (
	...
	mgmtCmd = &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			// Create certificate manager if Cloudflare config is available
			_ = createCertificateManager(config, "")
