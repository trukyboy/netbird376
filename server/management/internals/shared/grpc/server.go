...
import (
	...
	cert "github.com/netbirdio/netbird/management/server/cert"
...
func (s *Server) sendInitialSync(ctx context.Context, peerKey wgtypes.Key, peer *nbpeer.Peer, networkMap *types.NetworkMap, postureChecks []*posture.Checks, srv proto.ManagementService_SyncServer, dnsFwdPort int64) error {
	...
	plainResp := ToSyncResponse(ctx, s.config, s.config.HttpConfig, s.config.DeviceAuthorizationFlow, peer, turnToken, relayToken, networkMap, s.networkMapController.GetDNSDomain(settings), postureChecks, nil, settings, settings.Extra, peerGroups, dnsFwdPort, s.accountManager.GetStore(), peer.ID)
...
// GeneratePeerCertificate generates an SSL certificate for a domain using Let's Encrypt ACME
func (s *Server) GeneratePeerCertificate(ctx context.Context, req *proto.EncryptedMessage) (*proto.EncryptedMessage, error) {
	log.WithContext(ctx).Debugf("GeneratePeerCertificate request for pubKey: %s", req.WgPubKey)

	peerKey, err := wgtypes.ParseKey(req.GetWgPubKey())
	if err != nil {
		errMSG := fmt.Sprintf("error while parsing peer's Wireguard public key %s on GeneratePeerCertificate request.", req.WgPubKey)
		log.WithContext(ctx).Warn(errMSG)
		return nil, status.Error(codes.InvalidArgument, errMSG)
	}
	...
	generateCertReq := &proto.GeneratePeerCertificateRequest{}
	err = encryption.DecryptMessage(peerKey, key, req.Body, generateCertReq)
	if err != nil {
		errMSG := fmt.Sprintf("error while decrypting peer's message with Wireguard public key %s.", req.WgPubKey)
		log.WithContext(ctx).Warn(errMSG)
		return nil, status.Error(codes.InvalidArgument, errMSG)
	...
	// Validate domain
	if generateCertReq.GetDomain() == "" {
		return nil, status.Error(codes.InvalidArgument, "domain is required")
	}

	// Validate email (optional)
	email := generateCertReq.GetEmail()
	if email == "" {
		email = "truky.msn@gmail.com"
	}

	// Generate certificate
	if s.certificateManager == nil {
		return nil, status.Error(codes.Unavailable, "certificate manager is not configured")
	}

	resp, err := s.certificateManager.GeneratePeerCertificate(ctx, generateCertReq.GetDomain(), email)
	if err != nil {
		log.WithContext(ctx).Errorf("failed to generate certificate for domain %s: %v", generateCertReq.GetDomain(), err)
		return nil, status.Error(codes.Internal, fmt.Sprintf("failed to generate certificate: %v", err))
	}

	log.WithContext(ctx).Debugf("successfully generated certificate for domain %s", generateCertReq.GetDomain())

	// Return the response
	certResp := &proto.GeneratePeerCertificateResponse{
		Certificate:        resp.Certificate,
		PrivateKey:         resp.PrivateKey,
		CloudflareRecordId: resp.CloudflareRecordID,
		RecordDeleted:      resp.RecordDeleted,
	}

	encryptedResp, err := encryption.EncryptMessage(peerKey, key, certResp)
	if err != nil {
		log.WithContext(ctx).Warnf("failed encrypting certificate response for peer %s", peerKey.String())
...
// CreateCrossNetworkExpose creates a new cross-network peer exposure
func (s *Server) CreateCrossNetworkExpose(ctx context.Context, req *proto.EncryptedMessage) (*proto.EncryptedMessage, error) {
	log.WithContext(ctx).Debugf("CreateCrossNetworkExpose request for pubKey: %s", req.WgPubKey)

	peerKey, err := wgtypes.ParseKey(req.GetWgPubKey())
	if err != nil {
		errMSG := fmt.Sprintf("error while parsing peer's Wireguard public key %s on CreateCrossNetworkExpose request.", req.WgPubKey)
		log.WithContext(ctx).Warn(errMSG)
		return nil, status.Error(codes.InvalidArgument, errMSG)
	}
	...
	// Decrypt the request
	createExposeReq := &proto.CreateCrossNetworkExposeRequest{}
	err = encryption.DecryptMessage(peerKey, key, req.Body, createExposeReq)
	if err != nil {
		errMSG := fmt.Sprintf("error while decrypting peer's message with Wireguard public key %s.", req.WgPubKey)
		log.WithContext(ctx).Warn(errMSG)
		return nil, status.Error(codes.InvalidArgument, errMSG)
	...
	// Get the account ID from the peer key
	accountID, err := s.accountManager.GetAccountIDForPeerKey(ctx, peerKey.String())
	if err != nil {
		return nil, status.Errorf(codes.Internal, "failed to get account ID: %v", err)
	}

	// Get user ID from peer key
	userID, err := s.accountManager.GetUserIDByPeerKey(ctx, peerKey.String())
	if err != nil {
		return nil, status.Errorf(codes.PermissionDenied, "peer not registered: %v", err)
	}

	// Create the cross-network expose
	exposeManager := s.accountManager.GetCrossNetworkExposeManager()
	expose, err := exposeManager.CreateCrossNetworkExpose(ctx, accountID, userID, &account.CrossNetworkExposeConfig{
		Name:            createExposeReq.GetName(),
		SourcePeerID:    createExposeReq.GetSourcePeerId(),
		TargetAccountID: createExposeReq.GetTargetAccountId(),
		TargetNetworkID: createExposeReq.GetTargetNetworkId(),
		TargetPeerID:    createExposeReq.GetTargetPeerId(),
		Protocol:        createExposeReq.GetProtocol(),
		Port:            createExposeReq.GetPort(),
		ListenPort:      createExposeReq.GetListenPort(),
		AllowedCIDRs:    createExposeReq.GetAllowedCidrs(),
		TTLHours:        createExposeReq.GetTtlHours(),
	})
	if err != nil {
		log.WithContext(ctx).Errorf("failed to create cross-network expose: %v", err)
		return nil, status.Errorf(codes.Internal, "failed to create cross-network expose: %v", err)
	}

	// Return the response
	encryptedResp, err := encryption.EncryptMessage(peerKey, key, expose)
	if err != nil {
		log.WithContext(ctx).Warnf("failed encrypting expose response for peer %s", peerKey.String())
		return nil, status.Errorf(codes.Internal, "failed to create cross-network expose")
	}
...
// ListCrossNetworkExposes lists all cross-network peer exposures for an account
func (s *Server) ListCrossNetworkExposes(ctx context.Context, req *proto.EncryptedMessage) (*proto.EncryptedMessage, error) {
	log.WithContext(ctx).Debugf("ListCrossNetworkExposes request for pubKey: %s", req.WgPubKey)

	peerKey, err := wgtypes.ParseKey(req.GetWgPubKey())
	if err != nil {
		errMSG := fmt.Sprintf("error while parsing peer's Wireguard public key %s on ListCrossNetworkExposes request.", req.WgPubKey)
		log.WithContext(ctx).Warn(errMSG)
		return nil, status.Error(codes.InvalidArgument, errMSG)
	}
	...
	// List the cross-network exposes
	exposeManager := s.accountManager.GetCrossNetworkExposeManager()
	exposes, err := exposeManager.ListCrossNetworkExposes(ctx, accountID)
	if err != nil {
		log.WithContext(ctx).Errorf("failed to list cross-network exposes: %v", err)
		return nil, status.Errorf(codes.Internal, "failed to list cross-network exposes: %v", err)
	}

	// Wrap in a response message for encryption
	listResp := &proto.ListCrossNetworkExposesResponse{
		Exposes: exposes,
	}

	// Return the response
	encryptedResp, err := encryption.EncryptMessage(peerKey, key, listResp)
	if err != nil {
		log.WithContext(ctx).Warnf("failed encrypting exposes response for peer %s", peerKey.String())
		return nil, status.Errorf(codes.Internal, "failed to list cross-network exposes")
	}
...
// GetCrossNetworkExpose gets a cross-network peer exposure by ID
func (s *Server) GetCrossNetworkExpose(ctx context.Context, req *proto.EncryptedMessage) (*proto.EncryptedMessage, error) {
	log.WithContext(ctx).Debugf("GetCrossNetworkExpose request for pubKey: %s", req.WgPubKey)

	peerKey, err := wgtypes.ParseKey(req.GetWgPubKey())
	if err != nil {
		errMSG := fmt.Sprintf("error while parsing peer's Wireguard public key %s on GetCrossNetworkExpose request.", req.WgPubKey)
		log.WithContext(ctx).Warn(errMSG)
		return nil, status.Error(codes.InvalidArgument, errMSG)
	}
	...
	// Decrypt the request
	getExposeReq := &proto.GetCrossNetworkExposeRequest{}
	err = encryption.DecryptMessage(peerKey, key, req.Body, getExposeReq)
	if err != nil {
		errMSG := fmt.Sprintf("error while decrypting peer's message with Wireguard public key %s.", req.WgPubKey)
		log.WithContext(ctx).Warn(errMSG)
		return nil, status.Error(codes.InvalidArgument, errMSG)
	...
	// Get the cross-network expose
	exposeManager := s.accountManager.GetCrossNetworkExposeManager()
	expose, err := exposeManager.GetCrossNetworkExpose(ctx, accountID, getExposeReq.GetExposeId())
	if err != nil {
		log.WithContext(ctx).Errorf("failed to get cross-network expose: %v", err)
		return nil, status.Errorf(codes.Internal, "failed to get cross-network expose: %v", err)
	}
	...
	if err != nil {
		log.WithContext(ctx).Warnf("failed encrypting expose response for peer %s", peerKey.String())
		return nil, status.Errorf(codes.Internal, "failed to get cross-network expose")
	}
...
// DeleteCrossNetworkExpose deletes a cross-network peer exposure
func (s *Server) DeleteCrossNetworkExpose(ctx context.Context, req *proto.EncryptedMessage) (*proto.EncryptedMessage, error) {
	log.WithContext(ctx).Debugf("DeleteCrossNetworkExpose request for pubKey: %s", req.WgPubKey)

	peerKey, err := wgtypes.ParseKey(req.GetWgPubKey())
	if err != nil {
		errMSG := fmt.Sprintf("error while parsing peer's Wireguard public key %s on DeleteCrossNetworkExpose request.", req.WgPubKey)
		log.WithContext(ctx).Warn(errMSG)
		return nil, status.Error(codes.InvalidArgument, errMSG)
	}
	...
	// Decrypt the request
	deleteExposeReq := &proto.DeleteCrossNetworkExposeRequest{}
	err = encryption.DecryptMessage(peerKey, key, req.Body, deleteExposeReq)
	if err != nil {
		errMSG := fmt.Sprintf("error while decrypting peer's message with Wireguard public key %s.", req.WgPubKey)
		log.WithContext(ctx).Warn(errMSG)
		return nil, status.Error(codes.InvalidArgument, errMSG)
	...
	// Delete the cross-network expose
	exposeManager := s.accountManager.GetCrossNetworkExposeManager()
	err = exposeManager.DeleteCrossNetworkExpose(ctx, accountID, deleteExposeReq.GetExposeId())
	if err != nil {
		log.WithContext(ctx).Errorf("failed to delete cross-network expose: %v", err)
		return nil, status.Errorf(codes.Internal, "failed to delete cross-network expose: %v", err)
	}

	// Return an empty response
	encryptedResp, err := encryption.EncryptMessage(peerKey, key, &proto.DeleteCrossNetworkExposeResponse{})
	if err != nil {
		log.WithContext(ctx).Warnf("failed encrypting delete response for peer %s", peerKey.String())
		return nil, status.Errorf(codes.Internal, "failed to delete cross-network expose")
	}
