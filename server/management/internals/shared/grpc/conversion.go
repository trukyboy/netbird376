...
func ToSyncResponse(ctx context.Context, config *nbconfig.Config, httpConfig *nbconfig.HttpServerConfig, deviceFlowConfig *nbconfig.DeviceAuthorizationFlow, peer *nbpeer.Peer, turnCredentials *Token, relayCredentials *Token, networkMap *types.NetworkMap, dnsName string, checks []*posture.Checks, dnsCache *cache.DNSConfigCache, settings *types.Settings, extraSettings *types.ExtraSettings, peerGroups []string, dnsFwdPort int64, store interface{ GetCrossNetworkPeersConfig(ctx context.Context, peerID string) ([]*proto.CrossNetworkPeerConfig, error) }, peerID string) *proto.SyncResponse {
	...
	// Add cross-network peer configs
	crossNetworkPeers := getCrossNetworkPeers(ctx, store, peerID)
	response.NetworkMap.CrossNetworkPeers = crossNetworkPeers
	response.NetworkMap.CrossNetworkPeersEmpty = len(crossNetworkPeers) == 0

	return response
}

// getCrossNetworkPeers retrieves cross-network peer configs from the store
func getCrossNetworkPeers(ctx context.Context, store interface{ GetCrossNetworkPeersConfig(ctx context.Context, peerID string) ([]*proto.CrossNetworkPeerConfig, error) }, peerID string) []*proto.CrossNetworkPeerConfig {
	if store == nil {
		return []*proto.CrossNetworkPeerConfig{}
	}

	configs, err := store.GetCrossNetworkPeersConfig(ctx, peerID)
	if err != nil {
