...
// GeneratePeerCertificate requests an SSL certificate from the management server
func (c *GrpcClient) GeneratePeerCertificate(ctx context.Context, domain, email string) (string, string, error) {
	serverKey, err := c.getServerPublicKey()
	if err != nil {
		return "", "", fmt.Errorf("get server public key: %w", err)
	}

	mgmCtx, cancel := context.WithTimeout(ctx, time.Minute*5)
	defer cancel()

	message := &proto.GeneratePeerCertificateRequest{
		Domain: domain,
		Email:  email,
	}
	...
	encryptedMSG, err := encryption.EncryptMessage(*serverKey, c.key, message)
	if err != nil {
		return "", "", fmt.Errorf("encrypt cert request: %w", err)
	}

	resp, err := c.realClient.GeneratePeerCertificate(mgmCtx, &proto.EncryptedMessage{
		WgPubKey: c.key.PublicKey().String(),
		Body:     encryptedMSG,
	})
	if err != nil {
	...
	if err := encryption.DecryptMessage(*serverKey, c.key, resp.Body, certResp); err != nil {
		return "", "", fmt.Errorf("decrypt response: %w", err)
	}

	return certResp.Certificate, certResp.PrivateKey, nil
}
